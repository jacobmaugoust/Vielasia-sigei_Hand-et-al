setwd("D:/TXT/Studies/Université/OneDrive - umontpellier.fr/PhD/CURRENT WORK/Vielase bat skull/stats")
...
# Custom functions (with additional details to the Supp File 4for the align_rep_ppca and conv.pgls in their script part as #comments)
{
...
mrb_trees<-read.mrbayes("Vielase_comb_morph_TE_dated_mrb_trees_for_PGLS.tre")
trees<-list()
...
SR_phylo<-read.tree("SR_dated_tree.tre")
AM_phylo<-read.tree("Amador_dated_tree.tre")
...
############### II - GEOMETRIC MORPHOMETRICS OF SKULL AND MANDIBLE AFTER ARBOUR ET AL. 2019
############### II - 1 - Input, prepare, and treat our perform the Procrustes superimposition following Arbour et al. 2019

# Skull landmarks and sliding semi-landmarks
lmk_skull<-read.table("lmk_skull.txt",header=T,dec=",",sep="\t")
...
lmk_mandible<-read.table("lmk_mandible.txt",header=T,dec=",",sep="\t")
...
############### II - 2 - Perform pPCAs for both datasets on all phylogenies
...
############### II - 3 - Custom operations on the points resulting from the pPCAs
...
############### II - 4 - Get averages of each species position on each axis and of the % of variance explained by each axis
...
############### II - 5 - Get exhaustive information of each species position by computing their 
...
lmk_eco<-read.table("lmk_eco.txt",header=T,dec=",",sep="\t")
...
# postscript("results/def/eps/Fig. SX1 - Skull morphospaces on pPC1-pPC2 and pPC1-pPC3 with phylogenetic variation.eps")
...
# postscript("results/def/eps/Fig. X1 - Skull GMM.eps")
...
############### II - 6 - b - For mandible points

# postscript("results/def/eps/Fig. SX2 - Mandible morphospaces on pPC1-pPC2 and pPC3-pPC4 with phylogenetic variation.eps")
par(mfrow=c(2,3),mar=rep(3,4),mgp=c(2,0.5,0))
...
# Plot average points on pPC1 and pPC2
# postscript("results/def/eps/Fig. X2 - Mandible GMM.eps")
...
############### III - BONY LABYRINTH PARAMETERS INVESTIGATIONS USING DATA FROM DAVIES ET AL. 2013A,B
############### III - 1 - Get phylogenetic data at the Mammalia scale

# Input mammalian-scale phylogeny of Alvarez-Carretero et al. 2021
mamm_phylo<-read.tree("Alvarez-Carretero et al 2021 Nature_mammalian phylogeny.tre")
...
############### III - 2 - Investigations on basilar membrane, number of cochlear turns, and body mass (after Davies et al. 2013a)

# Get the data
ear_data<-read.table("ear_data.txt",header=TRUE,dec=",",sep="\t")

############### III - 2 - a - Mixing phylogenetic data at the Mammalia and Chiroptera scales for the given dataset
...
############### III - 2 - b - Compute PGLS between log basilar membrane length and log body masses for all phylogenies
...
write.table(groups_PGLS_TT,file="results/def/tables/Table SX1 - p-values of between-groups and all-vs-groups PGLS regression parameter t-tests.txt",append=FALSE,quote=FALSE,sep="\t")

############### III - 2 - c - Regression graphics with the different groups regression lines
...
for (i in 1:5){
  if(i==1){
    postscript("results/def/eps/Fig. X3 - Basilar membrane regression.eps")
  }
  if(i==2){
    # postscript("results/def/eps/Fig. SX3 - Basilar membrane regressions.eps")
...
############### III - 2 - d - Calculating low and high frequency limits for Vielasia sigei

freq_ear_data<-read.table("frequency limits.txt",header=TRUE,dec=",",sep="\t")
...
# postscript("results/def/eps/Fig. S1-X - Frequency limits after Davies et al 2013a.eps")
...
for(i in 1:4){
  ...
  # segments(curr_x_Vielasia,curr_y_Vielasia[1],curr_x_Vielasia,curr_y_Vielasia[2],col="red",lwd=3)
...
freqs_davies
freqs_Vielasia
rbind("average davies estimation"=apply(freqs_davies,2,mean),"average Vielasia estimation"=apply(freqs_Vielasia,2,mean))

############### III - 3 - Investigations on the semicircular canals sizes vs cochlea size relationships

# Get the data
bony_lab<-read.table("bony_lab.txt",header=TRUE,sep="\t",dec=",")
bony_lab<-na.omit(bony_lab)

############### III - 3 - a - Mixing phylogenetic data at the Mammalia and Chiroptera scales for the given dataset
...
############### III - 3 - b - Simply reporting boxplots of semicircular canal size and cochlea size for extant bats
...
# postscript("results/def/eps/Fig. X4 - Bats canals and cochlea sizes.eps")
layout(matrix(ncol=4,nrow=1,c(1,1,1,2),byrow=TRUE))
par(pty="m",mar=c(5.1,4.6,4.1,8.1))
boxgroups(bony_lab_bats[,2:4],factor(bony_lab_bats$SuborderEcho),box.width=0.3,points.opt=list("col"=c(echo_cols,"red"),"bg"=c(echo_cols,"red"),"pch"=21,"lwd"=2),names=c("Anterior canal","Lateral canal","Posterior canal"),box.opt=list("frame"=FALSE,"yaxt"="n"))
axis(2,at=seq(0,2.5,0.5))
axis(2,mean(range(unlist(bony_lab_bats[,2:4]))),labels="Radius",tick=FALSE,cex.axis=1)
axis(4,mean(range(unlist(bony_lab_bats[,2:4]))),labels="Size",tick=FALSE,padj=5)
par(pty="m",mar=c(5.1,0.1,4.1,0.1))
boxgroups(as.matrix(bony_lab_bats[,5]),factor(bony_lab_bats$SuborderEcho),box.width=0.3,x.gap=0.1,points.opt=list("col"=c(echo_cols,"red"),"bg"=c(echo_cols,"red"),"pch"=21,"lwd"=2),names=c("Cochlea"),box.opt=list("frame"=FALSE,"yaxt"="n"))
axis(2,at=seq(1,5,1))
# dev.off()

# postscript("results/def/eps/Fig. X4 - Bats canals and cochlea sizes_test1.eps")
layout(matrix(ncol=4,nrow=1,c(1,1,1,2),byrow=TRUE))
par(pty="m",mar=c(2,12.1,1,5.1),las=1,cex.axis=2)
...
par(pty="m",mar=c(2,0.1,1,5.1))
boxgroups(as.matrix(bony_lab_bats[,5]),factor(bony_lab_bats$SuborderEcho),box.width=0.3,x.gap=0.05,points.opt=list("col"=c(echo_cols,"red"),"bg"=c(echo_cols,"red"),"pch"=21,"lwd"=3,cex=2),names=c("Cochlea"),box.opt=list("frame"=FALSE,"yaxt"="n",cex=2))
axis(2,at=seq(1,5,1),cex.axis=1)
# dev.off()

# postscript("results/def/eps/Fig. X4 - Bats canals and cochlea sizes_test2.eps")
layout(matrix(ncol=4,nrow=1,c(1,1,1,2),byrow=TRUE))
par(pty="m",mar=c(2,15.1,1,8.1),las=1,cex.axis=2)
boxgroups(bony_lab_bats[,2:4],factor(bony_lab_bats$SuborderEcho),box.width=0.3,points.opt=list("col"=c(echo_cols,"red"),"bg"=c(echo_cols,"red"),"pch"=21,"lwd"=3,cex=2),names=c("ASC","LSC","PSC"),box.opt=list("frame"=FALSE,"yaxt"="n",cex=2))
axis(2,cex.axis=1)
axis(2,mean(range(unlist(bony_lab_bats[,2:4]))),labels="Radius",tick=FALSE,cex.axis=2)
axis(4,mean(range(unlist(bony_lab_bats[,2:4]))),labels="Size",tick=FALSE,hadj=-1,cex.axis=2)
par(pty="m",mar=c(2,1.1,1,6.1))
boxgroups(as.matrix(bony_lab_bats[,5]),factor(bony_lab_bats$SuborderEcho),box.width=0.3,x.gap=0.05,points.opt=list("col"=c(echo_cols,"red"),"bg"=c(echo_cols,"red"),"pch"=21,"lwd"=3,cex=2),names=c("Cochlea"),box.opt=list("frame"=FALSE,"yaxt"="n",cex=2))
axis(2,at=seq(1,5,1),cex.axis=1)
# dev.off()

# postscript("results/def/eps/Fig. X4 - Bats canals and cochlea sizes_test3.eps")
...
############### III - 3 - c - Compute PGLS between each log semicircular canal size and log cochlea size for each 
...
############### III - 3 - d - Perform a discriminant analysis taking into account phylogeny (pFDA) and taking not it (LDA)
...
############### III - 3 - e - Get average results for each discriminant analysis
...
write.table(avg_confusion_all,file="results/def/tables/Table SX2 - Average confusion matrix for LDA and pFDA on canals vs cochlea size PGLS residuals.txt",append=FALSE,quote=FALSE,sep="\t")
...
write.table(avg_test.results_all,file="results/def/tables/Table SX3 - Average predictions of the LDA and of the pFDA for the pertaining of Vielasia sigei.txt",append=FALSE,quote=FALSE,sep="\t")
...
############### III - 3 - f - Get exhaustive information of each species position on the two first discriminant axes
...
# postscript("results/def/eps/Fig. SX4 - Whole canals vs cochlea sizes residuals LDA and pFDA.eps")
par(mfrow=c(2,3),mar=rep(3,4),mgp=c(2,0.5,0))
...
# postscript("results/def/eps/Fig. X5 - Canals vs cochlea sizes residuals LDA.eps")
...
# Plotting average position of the species on the two first discriminant axes
# postscript("results/def/eps/Fig. X6 - Canals vs cochlea sizes residuals pFDA.eps")
